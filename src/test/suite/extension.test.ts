...
suite('Extension Test Suite', () => {
  ...
  for (let i = 1; i <= 5; i++) {
    test(`Split Command Line Test - Iteration ${i}`, async () => {
      const commandLine =
        'git show fd766eba2b -- scripts/cmake/vcpkg_acquire_msys.cmake';
    ...
    test(`Split Command Line Test-2 - Iteration ${i}`, async () => {
      ...
      const splitCommandLine = await vscode.env.clipboard.readText();
      const expected = `"xvfb-run", "-s", "'-screen 0 1920x1080x24'", "node", "./out/test/runTest.js"`;
  ...
  test(`convert to win path`, async () => {
    const commandLine = `c:/windows/system32`;
    await vscode.env.clipboard.writeText(commandLine);

    await vscode.commands.executeCommand('copy-to-open.toWinPath');

    await new Promise((resolve) => setTimeout(resolve, 50));

    const splitCommandLine = await vscode.env.clipboard.readText();
    const expected = `c:\\windows\\system32`;

    assert.strictEqual(splitCommandLine, expected);
  });

  test(`convert to linux path`, async () => {
    const commandLine = `c:\\windows\\system32`;

    await vscode.env.clipboard.writeText(commandLine);

    await vscode.commands.executeCommand('copy-to-open.toLinuxPath');
